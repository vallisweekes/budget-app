...
export default async function SettingsPageContent(props: {
	...
	const fallback = !budgetPlanId ? await getDefaultBudgetPlanForUser({ userId, username: sessionUsername }) : null;

	// If a plan wasn't explicitly provided, but the user has one, canonicalize to the scoped settings URL.
	if (!budgetPlanId && fallback) {
		redirect(`/user=${encodeURIComponent(sessionUsername)}/${encodeURIComponent(fallback.id)}/page=settings`);
	}

	// If a plan was provided, verify ownership; otherwise render settings in "no plan" mode.
	if (budgetPlanId) {
		const plan = await prisma.budgetPlan.findUnique({ where: { id: budgetPlanId }, select: { id: true, userId: true } });
		if (!plan || plan.userId !== userId) {
			const nextFallback = await getDefaultBudgetPlanForUser({ userId, username: sessionUsername });
			if (nextFallback) {
				redirect(`/user=${encodeURIComponent(sessionUsername)}/${encodeURIComponent(nextFallback.id)}/page=settings`);
			}
			// No plans at all; drop into "no plan" settings view.
			budgetPlanId = "";
		} else {
	...
	const settings: Settings = budgetPlanId
		? await getSettings(budgetPlanId)
		: {
			payDate: 27,
			monthlyAllowance: 0,
			savingsBalance: 0,
