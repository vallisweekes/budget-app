...
import { getMobileApiErrorMessage, useUpdateSettingsMutation } from "@/store/api";
...
export default function SettingsStrategyScreen({ navigation, route }: RootStackScreenProps<"SettingsStrategy">) {
  ...
  const save = async () => {
    try {
      ...
      Alert.alert("Could not save strategy", getMobileApiErrorMessage(err, "Please try again."));
    } finally {
      setSaving(false);
    }
  };
