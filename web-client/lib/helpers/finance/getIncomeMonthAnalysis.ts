...
export async function getIncomeMonthAnalysis({ budgetPlanId, year, month, payFrequency, mode = "full" }: Params) {
	...
	const cadence = normalizePayFrequency(payFrequency ?? payPeriodContext.payFrequency);
	...
	const payDate = Number(payPeriodContext.payDate ?? plan?.payDate ?? 27);
	const payAnchorDate = cadence === "monthly" ? null : payPeriodContext.payAnchorDate;
	...
	const expenseSnapshotPromise = !isHomeCoreMode
	...
	const [periodIncomeItems, allocationSnapshot, customAllocationsSnapshot, debtPlan, plannedDebtOnly, expenseSnapshot] = await Promise.all([
	...
	const incomeItems = periodIncomeItems ?? [];
