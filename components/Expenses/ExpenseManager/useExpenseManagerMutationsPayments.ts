...
export function useExpenseManagerMutationsPayments(args: {
	...
	const handleTogglePaid = (expenseId: string) => {
		...
		startTransition(async () => {
			try {
				await actions.togglePaidAction(budgetPlanId, month, expenseId, year);
				router.refresh();
			} catch (err) {
	...
	const handleApplyPayment = (expenseId: string) => {
		...
		startTransition(async () => {
			try {
				const res = await actions.applyExpensePaymentAction(budgetPlanId, month, expenseId, paymentAmount, year);
				if (!res?.success) {
					if (prevSnapshot) {
