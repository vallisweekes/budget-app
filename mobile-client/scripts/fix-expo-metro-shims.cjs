...
const expoMetroConfigCompatRoot = path.join(expoPackageRoot, 'metro-config');
const nestedExpoMetroConfigRoot = path.join(expoPackageRoot, 'node_modules', '@expo', 'metro-config');
...
function ensureExpoMetroConfigCompat() {
  if (!fileExists(nestedExpoMetroConfigRoot)) {
    return 0;
  }
  ...
  const indexShim =
    `// Auto-generated by scripts/fix-expo-metro-shims.cjs\n` +
    `module.exports = require('../node_modules/@expo/metro-config');\n`;
  if (ensureFile(path.join(expoMetroConfigCompatRoot, 'index.js'), indexShim)) {
    createdCount += 1;
  }

  const transformWorkerShim =
    `// Auto-generated by scripts/fix-expo-metro-shims.cjs\n` +
    `module.exports = require('../../../node_modules/@expo/metro-config/build/transform-worker/supervising-transform-worker');\n`;
  if (ensureFile(path.join(expoMetroConfigCompatRoot, 'build', 'transform-worker', 'supervising-transform-worker.js'), transformWorkerShim)) {
    createdCount += 1;
  }
...
function main() {
  ...
  createdCount += ensureExpoMetroConfigCompat();
