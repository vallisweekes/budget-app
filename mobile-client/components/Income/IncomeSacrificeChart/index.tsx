...
export default function IncomeSacrificeChart({ rows, currency }: IncomeSacrificeChartProps) {
  ...
  const values = useMemo(() => rows.map((row) => Number(row.value) || 0), [rows]);
  ...
  const yValue = (value: number) => topPad + plotH - (value / maxVal) * plotH;
  ...
  const tooltipX = activePoint
    ? Math.min(chartW - rightPad - tooltipW, Math.max(leftPad, activePoint.x - tooltipW / 2))
    : 0;
    ...
    <View style={styles.wrap}>
      <Svg width={chartW} height={chartH}>
        ...
        {points.map((point, index) => (
          <React.Fragment key={rows[index]!.key}>
            {activeIndex === index ? <Circle cx={point.x} cy={point.y} r={11} fill={T.accent} fillOpacity={0.18} /> : null}
            <Circle
              cx={point.x}
              cy={point.y}
            ...
            />
            {activeIndex === index ? <Circle cx={point.x} cy={point.y} r={3} fill={T.onAccent} /> : null}
          </React.Fragment>
        ))}
        ...
        {activePoint ? (
          <>
            ...
            <Path
              ...
              opacity={0.96}
            />
            <SvgText x={tooltipX + tooltipW / 2} y={tooltipY + 22} fill={T.bg} fontSize={15} fontWeight="900" textAnchor="middle">
      ...
      <View style={styles.xLabels}>
        {rows.map((row) => (
          <Text key={row.key} style={styles.xLabelText}>{row.label}</Text>
        ))}
      </View>
    </View>
  );
